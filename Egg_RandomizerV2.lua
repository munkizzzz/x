...
-- 🔧 Set Equipped Pet Age to 50 (Visual Only)
local ageButton = Instance.new("TextButton", frame)
ageButton.Size = UDim2.new(1, -20, 0, 30)
ageButton.Position = UDim2.new(0, 10, 0, 180)
ageButton.BackgroundColor3 = Color3.fromRGB(120, 100, 200)
ageButton.Text = "⏳ Set Equipped Pet Age to 50"
ageButton.TextSize = 16
...
ageButton.MouseButton1Click:Connect(function()
    ...
    for _, pet in pairs(Workspace:GetDescendants()) do
        if pet:IsA("Model") and pet:FindFirstChild("Age") and (pet:GetModelCFrame().Position - root.Position).Magnitude < 15 then
            ...
            if label and label:FindFirstChildOfClass("TextLabel") then
                ...
                if oldText:match("Age %d+") then
                    textLabel.Text = oldText:gsub("Age %d+", "Age 50")
                else
                    textLabel.Text = oldText .. " | Age 50"
                end
            end
        end
    end
